...
class VirTable extends React.PureComponent {
  ...
  componentDidMount() {
    ...
    const avgColumnWidth = this.getAvgColumnWidth();
    const columns = this.createColumns();
    this.setState({ rsState: { ...rsState, columnWidth: avgColumnWidth }, columns });
  }
  ...
  createColumns = () => {
    ...
    this.props.columnNames.map((columnName) => {
      newColumns.push({ width: this.props.columnWidth, left: totalLeft, name: columnName });
      totalLeft += this.props.columnWidth;
    });
  ...
  getResultComponent = () => {
    console.log('getResultComponent: ', this.state.columns.length, this.props.rows.length);
    console.log('getResultComponent: ', this.state.columns);

    if (this.state.columns.length <= 0 || this.props.rows.length <= 0) {
